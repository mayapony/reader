...
export async function translationByBaidu(text: string) {
  const params = {
    ...
    get sign() {
      return md5(this.appid + this.q + this.salt + process.env.EXPO_PUBLIC_BAIDU_KEY).toString()
    },
  }
  ...
  const response = await fetch(
    `https://fanyi-api.baidu.com/api/trans/vip/translate?${new URLSearchParams(params)}`,
    {
      method: 'POST',
    },
  )
