...
const getCoverImage = async (zip: JSZip, metadata: any, opfPath: string) => {
  try {
    ...
    if (!coverId) {
      coverId = metadata.manifest.item.find((i: any) => i.properties === 'cover-image')?.id
    }
...
export const getMetadataFromEpub = async (epubUri: string) => {
  try {
    ...
    const title = metadata['dc:title']?.['#text'] || metadata['dc:title'] || '未知书名'
    const author = metadata['dc:creator']?.['#text'] || metadata['dc:creator'] || '未知作者'
